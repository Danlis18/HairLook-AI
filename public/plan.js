...
function logPaddleEvent(event){
  ...
  const metadata={
    ...
    code:String(event?.code||'').slice(0,160),
    detail:String(event?.detail||'').slice(0,500),
    documentationUrl:String(event?.documentation_url||'').slice(0,500)
  };
...
function handlePaddleEvent(event){
  ...
  if(event?.name==='checkout.warning'){
    console.warn('paddle_checkout_warning',event);
  }
  ...
  try{
    ...
    Paddle.Checkout.open({
      ...
      settings:{
        ...
        locale:'en',
        allowLogout:false,
        successUrl:`${location.origin}/dashboard`
      }
    });
    ...
    const messages={
      ...
      paddle_environment_mismatch:'Paddle live/sandbox configuration does not match. Please contact support.',
      paddle_price_invalid:'The Paddle price configuration is invalid. Please contact support.',
