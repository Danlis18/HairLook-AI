...
router.post('/paddle', async (req, res, next) => { try {
  ...
  if (!verified.ok) {
    log.warn('paddle_webhook_rejected', { reason:verified.reason });
  ...
  if (!eventId || !eventType) {
    log.warn('paddle_webhook_missing_id_or_type', { eventId,eventType });
    return res.status(200).send('ok');
  }

  const isAdjustment = eventType.startsWith('adjustment.');
  const safePayload = isAdjustment ? sanitizeAdjustmentPayload(data) : sanitizeTransactionPayload(data);
  ...
  if (eventType === 'transaction.completed') {
    const leadId = String(data.custom_data?.lead_id || '');
    const leadSig = String(data.custom_data?.lead_sig || '');
    if (!verifyLeadCorrelation(leadId, leadSig)) {
      log.warn('paddle_bad_lead_correlation', { transactionId:data.id });
      return res.status(200).send('ok');
    }

    const purchasedPriceIds = transactionPriceIds(data);
    if (!purchasedPriceIds.includes(config.paddlePriceId)) {
      log.warn('paddle_price_mismatch', { transactionId:data.id, purchasedPriceIds });
      return res.status(200).send('ok');
    }

    const lead = await repo.getLead(leadId);
    if (!lead) {
      log.warn('paddle_unknown_lead', { leadId,transactionId:data.id });
      return res.status(200).send('ok');
    }
    ...
    if (!payment) {
      log.warn('paddle_adjustment_unknown_transaction', { transactionId:data.transaction_id });
      return res.status(200).send('ok');
    }
    ...
    if (status !== payment.status) {
      await repo.upsertPayment({
        ...
        raw_payload:sanitizeAdjustmentPayload(data)
      });
