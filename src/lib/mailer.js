...
export async function sendMagicLink({to,url,admin=false}){
  ...
  const html=emailShell({
    ...
    lead:`Your secure ${config.productName} access is ready. Use the button below to sign in automatically — no password or second email entry is required.`,
    content:`<tr><td style="padding:10px 34px 22px"><div style="background:#f3efe6;border:1px solid #e4dccf;border-radius:18px;padding:18px 20px;font-family:Arial,sans-serif;font-size:14px;line-height:1.65;color:#536159"><strong style="color:#26342e">Why did I receive this?</strong><br>You requested a secure sign-in link for your ${config.productName} private order. The link expires in ${config.magicLinkTtlMinutes} minutes and works once. If this wasn't you, simply ignore this message.</div></td></tr>`,
...
export async function sendVerificationCode({to,code}){
  ...
  const html=emailShell({
    ...
    lead:`Enter this 6-digit code on the ${config.productName} verification screen to continue securely to your order.`,
...
export async function sendResultsReady({to}){
  ...
  const html=emailShell({
    ...
    lead:`Your private ${config.productName} hairstyle collection is ready to view.`,
    content:'',
